...
interface ResponsiveHologramViewerProps {
  modelUrl: string;
  targetMuscle?: string;
...
`,
  ...
  return (
    <div className={`relative ${isMobile ? 'w-screen h-screen fixed inset-0 z-50' : 'w-full h-full'}`}>
      ...
      <div className={`absolute bottom-2 left-1/2 -translate-x-1/2 w-full px-4 ${isMobile ? 'px-2' : 'px-4'}`}>
        {suggestedView && (
          <div className="flex justify-center mb-2">
            <div
              className="bg-spartan-card/80 backdrop-blur-sm text-spartan-text-secondary font-semibold rounded-full flex items-center gap-2 animate-fadeIn"
              style={{
                fontSize: suggestionFontSize,
                padding: `${suggestionPaddingY} ${suggestionPaddingX}`
              }}
            >
              <BrainIcon className="text-spartan-gold" style={{ width: `${16 * densityFactor}px`, height: `${16 * densityFactor}px` }} />
              <span>Sugerencia del Coach: {viewSuggestionMap[suggestedView]}</span>
            </div>
          </div>
        )}
        <div
          className="flex justify-center items-center gap-2 bg-spartan-card/50 backdrop-blur-sm rounded-lg"
          style={{
            padding: `${4 * densityFactor}px`,
            gap: `${8 * densityFactor}px`
          }}
        >
          <button
            onClick={() => setView('frontal', new THREE.Vector3(0, 1.2, 2.5), new THREE.Vector3(0, 1, 0))}
            className={getButtonClass('frontal')}
            style={{
          ...
          <button
            onClick={() => setView('lateral', new THREE.Vector3(2.5, 1.2, 0), new THREE.Vector3(0, 1, 0))}
            className={getButtonClass('lateral')}
            style={{
              fontSize: buttonFontSize,
              padding: `${buttonPaddingY} ${buttonPaddingX}`
            }}
          ...
          <button
            onClick={() => setView('superior', new THREE.Vector3(0, 3, 0.1), new THREE.Vector3(0, 1, 0))}
            className={getButtonClass('superior')}
            style={{
              fontSize: buttonFontSize,
              padding: `${buttonPaddingY} ${buttonPaddingX}`
            }}
...
export default ResponsiveHologramViewer;
